...
const FormUser = ({ createUser, userSelected, setUserSelected, updateUser, setFormIsOpen, formIsOpen }) => {
	const {
		handleSubmit,
		register,
		reset,
	...
	return (
		<div className={`form__container flex-container ${formIsOpen || 'form__close'}`}>
			<form onSubmit={handleSubmit(submit)} className="form flex-container">
				...
				<h2 className="form__title">{userSelected ? 'Update User Form' : 'Create User Form'}</h2>
				<div className="form__list">
					...
					<label className="form__field grid-container">
						...
						<input
							className={`form__input ${errors.birthday ? 'form__input--error' : ''}`}
							type="date"
							{...register('birthday', {
								maxLength: {
									value: 10,
					...
					<label className="form__field grid-container">
						...
						<input
							className={`form__input ${errors.image_url ? 'form__input--error' : ''}`}
							type="text"
							{...register('image_url', {
								required: 'This field is required',
							})}
